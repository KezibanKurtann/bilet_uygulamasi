        ...
        sc = $('#bus-seat-map').seatCharts({ 
            ...
            click: function() {
                if (this.status() == 'available') {
                    ...
                    $('<li>' + this.data().category + ' Koltuk # ' + this.settings.label + ': <b>$' + this.data().price + '</b> <a href="#" class="cancel-cart-item">[cancel]</a></li>')
...
$(function() {
    $('#checkout-button').click(function() {
        ...
        if (items.length <= 0) {
            alert("Lütfen önce en az 1 koltuk seçin.")
            return false;
        }
        var selected = [];
        ...
        alert("Koltuklar başarıyla Rezerve edildi.")
        location.reload()
    })
    $('#reset-btn').click(function() {
        if (confirm("Otobüs rezervasyonunu sıfırlamak istediğinizden emin misiniz?") === true) {
            localStorage.removeItem('booked')
            alert("Rezervasyon başarıyla Sıfırlandı.")
            location.reload()
        }
    })
})